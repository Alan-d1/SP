...
class Coco(data.Dataset):
    ...
    def __init__(self, export=False, transform=None, task='train', **config):
        if export:
            self.had_npz = os.listdir("./logs/magicpoint_synth_homoAdapt_coco/predictions/" + task)
        else:
            self.had_npz = []
        pass
    ...
    def __getitem__(self, index):
        ...
        def _read_RGBimage(path):
            ...
            # # 标准化（减去均值，除以标准差）
            # mean = np.array([0.485, 0.456, 0.406], dtype=np.float32)  # 均值
            # std = np.array([0.229, 0.224, 0.225], dtype=np.float32)  # 标准差
        ...
        def imgPhotometric(img, type='Grey'):
            """
        ...
        sample = self.samples[index]  # data path sample = {'image': img, 'name': name}
        ...
        if self.labels:
            ...
            if self.config['warped_pair']['enable']:
                ...
                # warp original image
                warped_img = torch.tensor(img_o, dtype=torch.float32)  # HW
                warped_img = self.inv_warp_image(warped_img.squeeze(), inv_homography, mode='bilinear').unsqueeze(
                    0)  # 1HW
                if (self.enable_photo_train == True and self.action == 'train') or (
                        self.enable_photo_val and self.action == 'val'):
                    warped_img = imgPhotometric(warped_img.numpy().squeeze())  # numpy array (H, W, 1)
                    warped_img = torch.tensor(warped_img, dtype=torch.float32)  # (H, W, 1)
                    pass
                warped_img = warped_img.view(-1, H, W)  # (1, H, W)
                ...
                # process RGB image
                warped_RGBimg = torch.tensor(img_RGB, dtype=torch.float32)  # HWC
                R = warped_RGBimg[:, :, 0]  # 获取红色通道HW1
                G = warped_RGBimg[:, :, 1]  # 获取绿色通道
                B = warped_RGBimg[:, :, 2]  # 获取蓝色通道
                warped_R = self.inv_warp_image(R.squeeze(), inv_homography, mode='bilinear').unsqueeze(0)  # 1HW
                warped_G = self.inv_warp_image(G.squeeze(), inv_homography, mode='bilinear').unsqueeze(0)
                warped_B = self.inv_warp_image(B.squeeze(), inv_homography, mode='bilinear').unsqueeze(0)
                if (self.enable_photo_train == True and self.action == 'train'):
                    img = torch.cat([warped_R, warped_G, warped_B], dim=0)  # 3HW
                    # 然后转置为 (H, W, 3)
                    warped_RGBimg = img.permute(1, 2, 0)
                    warped_RGBimg = imgPhotometric(warped_RGBimg.numpy().squeeze(), 'RGB')  # numpy array (H, W, 3)
                    warped_RGBimg = torch.tensor(warped_RGBimg, dtype=torch.float32)  # (H, W, 3)
                    warped_RGBimg = warped_RGBimg.permute(2, 0, 1)
                    pass
                if (self.action == 'val'):
                    warped_RGBimg = warped_RGBimg.permute(2, 0, 1)

                input.update({'warped_RGBimg': warped_RGBimg})
