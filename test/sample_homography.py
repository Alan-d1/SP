...
import cv2
from evaluations.utils.utils import warp_points_np
...
def test_sample_homography():
  ...
  if test_tf == True:
    from evaluations.utils import sample_homography as sample_homography
    boundary = 1
  ...
  if test_corner_def:
    corner_img = np.array([(-1, -1), (-1, 1), (1, 1), (1, -1)])
    from evaluations.utils import sample_homography_np as sample_homography
    boundary = 2
    ...
    from evaluations.utils.utils import sample_homography
...
def test_valid_mask():
  from evaluations.utils.utils import pltImshow
  batch_size = 1
  ...
  from evaluations.utils.utils import compute_valid_mask, labels2Dto3D
  device = 'cpu'
